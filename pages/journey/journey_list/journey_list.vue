    ...
    <view class="eva-section" @click="toCallBack()" v-else>
      ...
      <view class="eva-box" v-for="(item, index) in callbackList" :key="index">
        ...
        <view class="right">
          ...
          <view class="bot">
            <text class="attr">购买类型：{{ item.jitem.name }}</text>
...
export default {
  ...
  async onLoad(options) {
    ...
    if (options.judge == 1) {
      let id = options.id;
      let exam = await this.$apis.jitem.findOneById(id);
      var call = await this.$apis.cart.findJourneyCallBack(id, 0, 1);
      var fav = await this.$apis.favorite.findJourney(uid, id);
      this.item = exam.data;
    } else {
      this.item = JSON.parse(options.item);
      var call = await this.$apis.cart.findJourneyCallBack(this.item.id, 0, 1);
      var fav = await this.$apis.favorite.findJourney(uid, this.item.id);
    }
    this.callbackNumber = call.data.count || 0;
    this.callbackList = call.data.rows.filter(item => {
      item = Object.assign(item, this.orderTimeExp(item.updated_at));
      return item;
    });
  ...
  methods: {
    ...
    async toFavorite() {
      ...
      if (this.favorite) {
        ...
        // 未加入收藏，添加
        let addFav = await this.$apis.favorite.createJourney(1, jid, uid);
    ...
    buy() {
      ...
      uni.navigateTo({ url: `/pages/order/createOrder?kind=0&type=4&data=${data}&other=${obj}` });
    },
    // 加入购物车
    async toCart() {
      const { condition } = this.item;
    ...
    // 评价
    toCallBack() {
      uni.navigateTo({ url: `../journey_callback/journey_callback?id=${this.item.id}` });
    },
    //时间格式化
    orderTimeExp(time) {
