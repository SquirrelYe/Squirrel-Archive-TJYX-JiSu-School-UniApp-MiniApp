      ...
      <view class="input-content">
        ...
        <view v-show="judgePhone">
          <view class="input-item">
            ...
            <input type="mobile" placeholder="6-16位不含特殊字符的数字、字母组合" placeholder-class="input-empty" maxlength="20" password v-model="password" />
          </view>
          <view class="input-item">
...
export default {
  ...
  onLoad() {
    ...
    uni.login({
      complete: function (res) {
        if (res.code) {
          ...
          wx_api.getOpenid(res).then(res => {
            console.log('获取openid', res.data);
            _this.openid = res.data.openid;
          });
          wx_api.getAccessTaken().then(res => {
            console.log('获取access_taken', res.data);
            _this.AccessTaken = res.data.access_token;
          });
        } else {
    ...
    // 获取学校列表
    apis.school.findAndCountAllOnlyName().then(res => {
  ...
  methods: {
    ...
    // 用户条款
    law(e) {
      uni.navigateTo({ url: 'law/law?judge=0' });
    }
  }
};
</script>
