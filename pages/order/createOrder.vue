  ...
  <view>
    ...
    <view class="goods-section">
      <view class="g-header b-b">
        <image class="logo" :src="host + '/' + item.logo"></image>
        <text class="name">{{ item.name }}</text>
      </view>
      <!-- 商品列表 -->
      <view class="g-item">
        <image :src="host + '/' + item.logo"></image>
        <view class="right">
          <text class="title clamp">{{ item.title }}</text>
          <text class="spec">{{ item.name }}</text>
    ...
    <view class="yt-list">
      <view class="yt-list-cell b-b">
        ...
        <text class="cell-tip active" v-if="!useTicket" @click="getTicket('show')"> 选择优惠券 </text>
    ...
    <view class="yt-list">
      ...
      <view class="yt-list-cell desc-cell">
        ...
        <input class="desc" type="text" v-model="good.other" placeholder="请填写备注信息" placeholder-class="placeholder" />
      </view>
    </view>
...
export default {
  ...
  onLoad(option) {
    ...
    if (this.kind == 0) {
      ...
      let type = this.good.type;
      if (type == 0) this.item = this.good.eitem;
      if (type == 1) this.item = this.good.jitem;
      if (type == 2) this.item = this.good.fitem;
    } else {
      this.$api.msg('传递参数有误！');
      uni.navigateBack();
    }
    console.log(this.item, this.good);
  },
  methods: {
    // 显示优惠券面板
    toggleMask(type) {
    ...
    async getTicket(type) {
      ...
      this.ticketList = tic.data.rows.filter(item => {
        ...
        if (item.condition == 0 && now <= final) {
          item = Object.assign(item, this.orderTimeExp(item.ticket.end));
          return item;
        }
      });
      console.log('获取优惠券信息', this.ticketList);
      if (this.ticketList.length != 0) this.toggleMask(type);
      else this.$api.msg('暂无可用优惠券喔~');
    },
    stopPrevent() {},
    // 选择优惠券
    chooseTicket(item) {
    ...
    // 放弃优惠券
    cancelTic() {
      (this.useTicket = null), (this.short = 0);
    },
    // 提交
    submit() {
      if (this.isSubmit) this.$api.msg('请不要重复提交喔~');
        ...
        if (!this.addressData.id) {
          this.$api.msg('啊哦，没有选择收货地址~');
          return;
        }
        if (!this.good.number || this.good.number == 0) {
          this.$api.msg('啊哦，数量不对喔~');
          return;
        } else {
