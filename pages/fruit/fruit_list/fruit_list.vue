...
export default {
  ...
  async onLoad(options) {
    ...
    if (options.judge == 1) {
      ...
      var fav = await this.$apis.favorite.findFruit(uid, id);
      this.item = exam.data;
    } else {
      this.item = JSON.parse(options.item);
      var call = await this.$apis.cart.findFruitCallBack(this.item.id, 0, 1);
      var fav = await this.$apis.favorite.findFruit(uid, this.item.id);
    }
  ...
  methods: {
    ...
    async toFavorite() {
      ...
      if (this.favorite) {
        ...
        // 未加入收藏，添加
        let addFav = await this.$apis.favorite.createFruit(2, fid, uid);
    ...
    // 评价
    toCallBack() {
      uni.navigateTo({ url: `../fruit_callback/fruit_callback?id=${this.item.id}` });
    },
    //时间格式化
    orderTimeExp(time) {
