...
export default {
  ...
  onLoad() {
    ...
    if (this.user.authen) {
      ...
      this.$apis.user.cusLogin(phone, pass).then(res => {
  ...
  methods: {
    ...
    confirm() {
      ...
      if (!data.name) {
        ...
        this.$api.msg('请上传学生证和一卡通');
        return;
      }
      // 提交认证信息
      else {
        console.log(this.user, this.authen, this.imgList);
        this.$apis.authen.creat(this.user.id, data.name, this.user.school_id, data.xuehao, data.phone, this.imgList[0], this.imgList[1]).then(res => {
    ...
    ViewImage(e) {
      let arr = [];
      for (let img in this.imgList) {
        arr.push(`${this.host}/${this.imgList[img]}`);
      }
      uni.previewImage({
        urls: arr,
    ...
    // 校园大使条款
    law(e) {
      uni.navigateTo({ url: '../../public/law/law?judge=1' });
    }
  }
};
</script>
